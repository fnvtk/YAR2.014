            ...
            try
            {
                ...
                foreach (var bot in bots)
                {
                    ...
                    if (!(bot.Diablo.ManualPosSize && !Settings.Default.ForceAutoPos))
                    {
                        ...
                        var x = (int)(Settings.Default.AutoPosDiabloW * dx * 2);  // get next position on X-Axis of the screen
                        ...
                        if (bot.Demonbuddy.MainWindowHandle != IntPtr.Zero)
                        {
                            RemoveWindowFrame(bot.Demonbuddy.MainWindowHandle);
                            RepositionWindow(bot.Demonbuddy.MainWindowHandle, (int)Settings.Default.AutoPosDiabloW + x + screen.WorkingArea.X,
                                              y + screen.WorkingArea.Y, (int)Settings.Default.AutoPosDiabloW,
                                              (int)Settings.Default.AutoPosDiabloH);
                        }
                    }
